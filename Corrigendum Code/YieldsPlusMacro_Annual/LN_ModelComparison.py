...
def CP_factor(exrets,fwdrate):
    ...
    #Regress & Predict
    model = sm.OLS(np.mean(exrets[:-12,:],axis=1),sm.add_constant(fwdrate[:-12,:]))
    model = model.fit()
...
if __name__ == "__main__":
 #%% Global Params
    ...
    data_path = '../../../../Data/MACRO/LNDataStore_Fixed.h5'
    data_set = 'LW' # LW or SLe
    rhs_vars_set = 'Large' # Small or Large
    run_tag = 'RHS_'+rhs_vars_set
    lag_macro = False

    HyperFreq = 5*12
    NN_Params = {'Dropout': [0.3, 0.5], 'l1l2': [1, 0.1, 0.01]}
    ...
    if lag_macro:
        FRED_data = pd.read_hdf(data_path, key='macro_'+data_set+'_lagged')
        run_tag +='_LaggedMacro'
    else:
        FRED_data = pd.read_hdf(data_path, key='macro_'+data_set)
    FRED_data.set_index(['Date'], inplace=True)
    FRED_groups = pd.read_hdf(data_path, key='macrogroups_'+data_set)


    # Assign into NN convention
    Xexog = fwdrate.values
    X = FRED_data.values
    Y = exrets.values
    A = np.transpose(FRED_groups.values)
...
# %% FRED factors + Term Structure - Select Models to be run.

    models = [NFB.NN1LayerExog, NFB.NN2LayerExog, NFB.NN3LayerExog,
              NFB.NN1LayerExog_ExogNet1Layer,
              NFB.NN2LayerExog_ExogNet1Layer, NFB.NN3LayerExog_ExogNet1Layer,
              NFB.NN1LayerExog_ExogNet,
              NFB.NN2LayerExog_ExogNet, NFB.NN3LayerExog_ExogNet]


    modelnames = ['NN1LayerExog', 'NN2LayerExog', 'NN3LayerExog',
                  'NN1LayerExog_ExogNet1Layer',
                  'NN2LayerExog_ExogNet1Layer', 'NN3LayerExog_ExogNet1Layer',
                  'NN1LayerExog_ExogNet',
                  'NN2LayerExog_ExogNet', 'NN3LayerExog_ExogNet']


#  Group Ensembles
    # models = [NFB.NN1LayerEnsemExog,NFB.NN1LayerEnsemExog_ExogNet,
    #           NFB.NN2LayerEnsemExog,NFB.NN2LayerEnsemExog_ExogNet,
    #           NFB.NN3LayerEnsemExog,NFB.NN3LayerEnsemExog_ExogNet]

    # modelnames=['NN1LayerEnsemExog','NN1LayerEnsemExog_ExogNet',
    #             'NN2LayerEnsemExog','NN2LayerEnsemExog_ExogNet',
    #             'NN3LayerEnsemExog','NN3LayerEnsemExog_ExogNet']
    ...
    for modelnum,modelfunc in enumerate(models):
        ...
        if usemultproc == 1 and (('NN' in modelnames[modelnum])) and ('Ensem' not in modelnames[modelnum]) and ('CP' not in modelnames[modelnum]):
            print("Use multproc")
            j = 1
        ...
        elif 'Ensem' in modelnames[modelnum] and usemultproc == 1:
            print("Use multproc")
            j = 1
            for i in OoS_indeces:
                start = time.time()
                ...
                j += 1
                output = multProcessOwnExog(modelfunc,ncpus,nMC,X[:i+1,:],Xexog[:i+1,:],Y[:i+1,:],
                                            A=A, params=NN_Params, refit=refit, dumploc=dumploc)
                val_loss[i, :] = np.array([output[k][1] for k in range(nMC)])
                Y_forecast[i, :, :] = np.concatenate([output[k][0] for k in range(nMC)],axis=0)
                tempsort = np.argsort(val_loss[i, :])
                ypredmean = np.mean(Y_forecast[i, tempsort[:nAvg], :], axis=0)
