...
def R2OOS(y_true, y_forecast):
    ...
    y_condmean = np.insert(y_condmean,0,np.nan) # lag by one period
...
if __name__ == "__main__":
    ...
    HyperFreq = 5*12
    NN_Params = {'Dropout': [0.2, 0.4], 'l1l2': [0.001, 0.01, 0.1]}
    ...
    VarSave["Y_True"] = Y
    VarSave['RF'] = X[:, 0]
    VarSave["Dates"] = list(dates.strftime("%Y-%m-%d"))
    ...
    for modelnum, modelfunc in enumerate(models):
        ...
        if usemultproc == 1 and (('NN' in modelnames[modelnum])
                                 or ('AutoEnc' in modelnames[modelnum])):
            print("Use multproc")
