...
def forecast_with_randomforest(X_train, Y_train):
    """
    Run Random forest on the training data to forecast the target variable.
    ...
    '''# Standardize the features -> Not necessary for Random Forest
    ...
    for i in range(n_targets):
        ...
        random_search = RandomizedSearchCV(
            base_model, param_distributions=param_distributions, n_iter=50, 
            cv=ps, random_state=42, n_jobs=-1
        )
        random_search.fit(X_train_features, Y_train_target[:, i])
        '''
        base_model.fit(X_train_features, Y_train_target[:, i])
        predictions[0, i] = base_model.predict(X_test)[0]
