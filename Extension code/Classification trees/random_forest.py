...
def forecast_with_randomforest(X_train, Y_train):
    """
    Run RandomForestRegressor on the training data to forecast the target variable.
    Uses an expanding window and reserves the last sample as test.
    """
    # Reserve the last observation for testing
    X_train_features = X_train[:-1, :]
    X_test = X_train[-1, :].reshape(1, -1)
    Y_train_target = Y_train[:-1, :]

    # Standardization is not strictly required for tree models,
    # but it may help if predictors are on very different scales.
    '''
    ...
    for i in range(n_targets):
        ...
        model.fit(X_train_features, Y_train_target[:, i])
        predictions[0, i] = model.predict(X_test)[0]
...
fwd_maturities = ["1 y", "2 y", "3 y", "4 y", "5 y", "6 y", "7 y", "8 y", "9 y", "10 y"]
...
print("Running Random Forest with expanding window...\n")

# === Run expanding window forecasts ===
for t in range(oos_start_idx, n_obs):
    y_preds[t, :] = forecast_with_randomforest(Xexog[:t + 1, :], Y[:t + 1, :])
